# Fitur Ubah Masa/Bulan - Bukpot Downloader

## Overview

Fitur ini memungkinkan pengguna untuk mengubah filter masa pajak (bulan dan tahun) secara otomatis pada portal CoreTax DJP.

## Alur Kerja

1. **Filter masa pajak** → **Tunggu 2 detik** → **Download otomatis dengan delay**

## Kode Utama

### 1. Fungsi Utama - `setupFilterMasaPajak(bulan, tahun)`
    ...
    return new Promise((resolve) => {
        ...
        simulateUserDropdownSelection().then(result => {
            if (result) {
                ...
                // Jalankan script download setelah filter berhasil
                setTimeout(() => {
                    runSimpleDownloadScript();
                }, 2000); // Tunggu 2 detik setelah filter selesai
...
### 2. Pencarian Dropdown - `findTaxPeriodDropdown()`
...
### 3. Membuka Dropdown - `openDropdown(dropdown)`
...
### 4. Pencarian Opsi Target - `findTargetOption(bulan, tahun)`
...
async function findTargetOption(bulan, tahun) {
    return new Promise((resolve) => {
        ...
        const searchOptions = () => {
            ...
            const targetText = `${bulan} ${tahun}`.toLowerCase();
            ...
            const targetItem = Array.from(items).find(item => {
                ...
                return text.includes(targetText) ||
                       (text.includes(bulan.toLowerCase()) && text.includes(tahun));
            });
...
### 5. Seleksi Opsi - `selectOption(option)`
    ...
    return new Promise((resolve) => {
        try {
            ...
            const checkSelection = setInterval(() => {
                ...
                const hasChanged = labelText.includes(bulan);
                console.log(`  - Label text: "${labelText}"`);
                console.log(`  - Label contains "${bulan}": ${hasChanged}`);
...
### 6. Script Download Otomatis - `runSimpleDownloadScript()`

```javascript
// Simple download script function
function runSimpleDownloadScript() {
    console.log('Content script: === STARTING SIMPLE DOWNLOAD SCRIPT ===');
    ...
    buttons.forEach((button, index) => {
        setTimeout(() => {
            ...
            button.click();
            console.log(`Content script: PDF terunduh ${index + 1}`);
        }, delay * index);
    });
}
```
...
1. **Kirim message** ke content script dengan `action: 'applyTaxPeriodFilter'`
2. **Sertakan parameter**:
   - `bulan`: Nama bulan (contoh: "Juli")
   - `tahun`: Tahun (contoh: "2025")
3. **Script akan**:
   - Mencari dropdown filter masa pajak
   - Membuka dropdown
   - Mencari opsi yang sesuai
   - Memilih opsi tersebut
   - Menunggu 2 detik
   - Menjalankan download otomatis
...
- ✅ **Deteksi otomatis** dropdown dengan berbagai metode
- ✅ **Simulasi user behavior** yang natural
- ✅ **Error handling** yang komprehensif
- ✅ **Logging detail** untuk debugging
- ✅ **Timeout handling** untuk mencegah hang
- ✅ **Download otomatis** setelah filter berhasil
- ✅ **Delay antar download** untuk menghindari server overload